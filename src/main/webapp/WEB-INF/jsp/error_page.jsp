...
<body>
	...
	<table id="main-container">
		...
		<%-- HEADER --%>
		<c:if test="${medRole.name eq 'admin'}">
			<%@ include file="/WEB-INF/jspf/adminMenu.jspf"%>
		</c:if>
		<c:if test="${medRole.name eq 'doctor'}">
			<%@ include file="/WEB-INF/jspf/doctorMenu.jspf"%>
		</c:if>
		<c:if test="${medRole.name eq 'nurse'}">
			<%@ include file="/WEB-INF/jspf/nurceMenu.jspf"%>
		</c:if>

		<c:if test="${medRole.name == null}">
			<a href="controller?command=logout" class="btn btn-primary"
				role="button"><fmt:message key="header_jspf.anchor.logout" /></a>
		</c:if>
		<%-- HEADER --%>
		...
		<tr>
			<td class="content">
				...
				<c:set var="code"
					...
					var="message"
					value="${requestScope['javax.servlet.error.message']}" /> <%-- this way we get the exception --%>
				<c:set var="exception"
				...
				</c:if> <%-- this way we print exception stack trace --%> <c:if
